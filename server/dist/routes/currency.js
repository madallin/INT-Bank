...
router.get('/api/v1/exchange-rates', async (req, res) => {
    try {
        ...
        await redis_1.redis.setex(CACHE_KEY, CACHE_TTL_SECONDS, JSON.stringify(payload));
        res.json(payload);
    }
    catch (err) {
...
router.get('/:code', async (req, res) => {
    ...
    try {
        const response = await axios_1.default.get(primaryUrl, { timeout: 5000 });
        res.json({ success: true, source: 'jsdelivr', data: response.data });
    }
    catch (err1) {
        console.warn(`Primary API failed, trying fallback...`);
        try {
            const fallbackResponse = await axios_1.default.get(fallbackUrl, { timeout: 5000 });
