...
router.put('/:userId/accept-tos', async (req, res) => {
    ...
    try {
        ...
        const result = await clientDb.query(`UPDATE utilizatori
...
router.get('/:userId/has-tos', async (req, res) => {
    ...
    try {
        clientDb = await req.pool.connect();
        const result = await clientDb.query(`SELECT termeniacceptati
       FROM utilizatori
       WHERE id = $1`, [userId]);
        if (result.rowCount === 0) {
            res.status(404).json({ error: 'Utilizatorul nu a fost găsit' });
...
router.post('/:userId/verify-pin', async (req, res) => {
    ...
    try {
        ...
        const match = await bcrypt_1.default.compare(pin, pinHash);
...
router.put('/:userId/set-pin', async (req, res) => {
    ...
    let clientDb;
    try {
        const hashedPin = await bcrypt_1.default.hash(codPin, SALT_ROUNDS);
        clientDb = await req.pool.connect();
        const result = await clientDb.query(`UPDATE utilizatori SET pincont = $1 WHERE id = $2 RETURNING id`, [hashedPin, userId]);
        if (result.rowCount === 0) {
            res.status(404).json({ error: 'Utilizatorul nu a fost găsit' });
            return;
        }
...
router.get('/:userId/has-approved', async (req, res) => {
    ...
    try {
        clientDb = await req.pool.connect();
        const result = await clientDb.query(`SELECT contaprobat
       FROM utilizatori
       WHERE id = $1`, [userId]);
        if (result.rowCount === 0) {
            res.status(404).json({ error: 'Utilizatorul nu a fost găsit' });
...
router.post('/:userId/create-account-and-card', async (req, res) => {
    ...
    try {
        const result = await (0, banking_1.createAccountAndCard)(userId, currency || 'RON', countryCode || 'RO');
        res.status(200).json({
            success: true,
            account: {
...
router.get('/:userId/cards', async (req, res) => {
    ...
    `;
        ...
        if (hasNullAccountId) {
            const accountRes = await client.query('SELECT id FROM conturiBancare WHERE userid = $1 ORDER BY id LIMIT 1', [userId]);
            if (accountRes.rowCount != null && accountRes.rowCount > 0) {
                fallbackAccountId = accountRes.rows[0].id;
            }
        }
        const cards = result.rows.map((row) => {
            const last4 = (0, crypto_1.safeExtractLast4)(row.numarcard) || '****';
            let expiry = null;
            try {
                expiry = (0, crypto_1.decryptAESGCM)(row.dataexpirare);
            }
            catch (e) {
...
router.get('/:userId/accounts/:accountId', async (req, res) => {
    ...
    try {
        const result = await client.query(`SELECT id, IBAN, sold, moneda FROM conturiBancare WHERE userid = $1 AND id = $2`, [userId, accountId]);
        if (result.rowCount === 0) {
...
router.get('/:userId/cards/:cardId/details', async (req, res) => {
    ...
    try {
        client = await req.pool.connect();
        const result = await client.query(`SELECT numarcard, cvv, dataexpirare, detinator
       FROM carduri
       WHERE id = $1 AND userid = $2`, [cardId, userId]);
        if (result.rowCount === 0) {
        ...
        let pan, cvv, expiry;
        try {
            pan = (0, crypto_1.decryptAESGCM)(row.numarcard);
            cvv = (0, crypto_1.decryptAESGCM)(row.cvv);
            expiry = (0, crypto_1.decryptAESGCM)(row.dataexpirare);
        }
        catch (e) {
...
router.post('/:userId/transfer', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            // 2️⃣ Cont destinatar
            const receiverRes = await client.query(`SELECT c.id, c.userid, c.IBAN, c.sold, c.moneda, u.nume as owner_name
         FROM conturiBancare c
         JOIN utilizatori u ON u.id = c.userid
         WHERE REPLACE(c.IBAN, ' ', '') = $1`, [iban]);
            ...
            const convertedAmount = await (0, currency_1.convertCurrency)(numericAmount, sender.moneda, receiver.moneda);
            ...
            const insertRes = await client.query(`INSERT INTO transferuri (expeditor, receptor, suma, moneda, motiv)
         VALUES ($1,$2,$3,$4,$5)
         RETURNING id, dataTransfer`, [sender.id, receiver.id, numericAmount, sender.moneda, reason]);
