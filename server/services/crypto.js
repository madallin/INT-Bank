...
const { AES_GCM_IV_BYTES, AES_GCM_TAG_BYTES } = require('../config/constants');
 ...
 * încearcă toate cheile din AES_KEYS (rotatie chei).
...
function decryptAESGCM(payload) {
  ...
  if (data.length <= AES_GCM_IV_BYTES + AES_GCM_TAG_BYTES) throw new Error('Encrypted payload too short');
...
/**
 * Utility: safeExtractLast4 - încearcă să decripteze panEncrypted; în caz de eroare
 * returnează null (sau '0000' dacă preferi). Nu arunca erori mari către client.
 */
