...
exports.createAccountAndCard = createAccountAndCard;
exports.decryptAESGCM = decryptAESGCM;
exports.panHash = panHash;
...
function randomDigits(len) {
    ...
    while (out.length < len) {
        const next = crypto_1.default.randomInt(0, 1e6).toString().padStart(6, '0');
        out += next;
    }
...
function generateCardPan() {
    while (true) {
        const middle = randomDigits(constants_1.CARD_LENGTH - constants_1.CARD_BIN.length - 1);
        const withoutCheck = constants_1.CARD_BIN + middle;
...
function generateExpiry(issueDate = new Date(), years = constants_1.DEFAULT_CARD_LIFETIME_YEARS) {
...
function generateToken(pan) {
    return crypto_1.default
        .createHash('sha256')
        .update(pan + Date.now().toString() + crypto_1.default.randomBytes(12).toString('hex'))
        .digest('hex')
        .slice(0, 32);
}
// ---------- IBAN helpers ----------
...
function generateIban(countryCode = 'RO') {
    ...
    const accountNumber = randomDigits(constants_1.IBAN_ACCOUNT_LENGTH);
    const rearranged = constants_1.BANK_CODE + accountNumber + countryCode + '00';
    const numeric = lettersToNumber(rearranged);
    const check = 98 - mod97(numeric);
    return `${countryCode}${check.toString().padStart(2, '0')}${constants_1.BANK_CODE}${accountNumber}`;
}
// ---------- AES-GCM encryption / decryption ----------
function encryptAESGCM(plain) {
    const iv = crypto_1.default.randomBytes(constants_1.AES_GCM_IV_BYTES);
    const cipher = crypto_1.default.createCipheriv('aes-256-gcm', AES_PRIMARY_KEY, iv);
...
function decryptAESGCM(any) {
    ...
    if (version !== 'v1')
        throw new Error('unsupported enc version');
    const data = Buffer.from(dataStr, 'base64');
    const iv = data.slice(0, constants_1.AES_GCM_IV_BYTES);
    const tag = data.slice(constants_1.AES_GCM_IV_BYTES, constants_1.AES_GCM_IV_BYTES + AES_GCM_TAG_BYTES);
    const ciphertext = data.slice(constants_1.AES_GCM_IV_BYTES + AES_GCM_TAG_BYTES);
...
function panHash(pan) {
    return crypto_1.default.createHmac('sha256', HMAC_KEY).update(pan).digest('hex');
}
async function createAccountAndCard(userId, currency = 'RON', countryCode = 'RO') {
    if (!Number.isInteger(userId) || userId <= 0)
        throw new Error('invalid userId');
    const client = await database_1.pool.connect();
    try {
        // Preluăm prenume + nume din utilizatori
        const userRes = await client.query('SELECT prenume, nume FROM utilizatori WHERE id = $1', [userId]);
        ...
        for (let attempt = 0; attempt < constants_1.MAX_RETRIES; attempt++) {
            ...
            try {
                ...
                const accountRes = await client.query(`INSERT INTO conturiBancare(userid, IBAN, moneda, sold)
           VALUES($1,$2,$3,0.0)
           RETURNING id, IBAN, moneda, sold`, [userId, iban, currency.toUpperCase()]);
                const cardRes = await client.query(`INSERT INTO carduri(userid, numarCard, CVV, dataExpirare, detinator, token, accountid)
           VALUES($1,$2,$3,$4,$5,$6,$7)
           RETURNING id, token`, [userId, encrypted_pan, encrypted_cvv, encrypted_expiry, holderName, token, accountRes.rows[0].id]);
